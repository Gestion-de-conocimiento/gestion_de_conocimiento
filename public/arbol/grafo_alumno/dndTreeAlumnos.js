...
async function draw_tree_rutas(error, treeData) {
    ...
    function zoom() {
        ...
        x_init = d3.event.translate[0];
        y_init = d3.event.translate[1];
        e_init = d3.event.scale;
    }
    ...
    var baseSvgStudent = d3.select("#contenedor").append("svg")
        .attr("width", viewerWidth)
        .attr("height", viewerHeight);

    baseSvgStudent.append("rect")
        .attr("width", "100%")
        .attr("height", "100%")
        .attr("fill", "white")
    ...
    async function update(source) {
        ...
        for (var i = 0; i < nodoss.length; i++) {
            ...
            if (nodoss[i].length > 10) {
                ...
                for(var j=0; j < palabras.length; j++)
                {
                 // console.log("palabras "+palabras[j])
                  //console.log("pActual "+pActual)
                  if( (pActual+" "+palabras[j]).length<=18||j==palabras.length-1)
                  {
                    pActual+=" "+palabras[j]
                    if( (pActual).length>=18||j==palabras.length-1)
                    {
                      node.select("#"+nodoss[i].replace(/\s/g, '_'))
                      .append('svg:tspan')
                      .attr('x', -ratio + 8)
        ...
        node.select("circle.nodeCircle")
            ...
            .style("fill", function (d) {
                ...
                /*for(var j = 0; j < clasificacion_colores_nodos.length; j++ )
        {
        ...
        multiParents.forEach(function (multiPair) {
            ...
            svgGroup.insert("line", "g")
                ...
                .attr('marker-end', function (d) {
                    baseSvgStudent.append("svg:defs").append("marker")
    ...
    var svgGroup = baseSvgStudent.append("g");
    ...
    if (x_init != null && y_init != null && e_init != null) {
...
async function draw_status_grafo(error, treeData) {
    ...
    async function updateDos(source) {
